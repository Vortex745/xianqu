...
import { View, Hide } from '@/icons/tw-icons.js'
...
const collapseContextKey = Symbol('collapse')
const ElCollapse = defineComponent({
  ...
  setup(props, { slots, emit, attrs }) {
    ...
    const toggle = (name) => {
      ...
      } else if (Array.isArray(active.value)) {
        active.value = active.value.includes(name)
          ? active.value.filter((item) => item !== name)
          : [...active.value, name]
      } else if (active.value === null || active.value === undefined) {
        active.value = [name]
      } else {
        active.value = active.value === name ? [] : [active.value, name]
      }
    ...
    return () => h('div', { class: ['el-collapse', attrs.class] }, slots.default ? slots.default() : [])
  }
})
...
const ElCollapseItem = defineComponent({
  ...
  setup(props, { slots, attrs }) {
    ...
    return () => {
      const opened = collapse?.isOpen(itemName.value) ?? false
      return h('section', { class: ['el-collapse-item', { 'is-active': opened }, attrs.class] }, [
        h(
          'button',
          {
            class: 'el-collapse-item__header',
            type: 'button',
            onClick: () => collapse?.toggle(itemName.value)
          },
          [
            h('span', { class: 'el-collapse-item__title' }, props.title || ''),
            h('span', { class: 'el-collapse-item__arrow' }, opened ? '−' : '+')
          ]
        ),
        h(
          ...
          null,
          {
            default: () =>
              opened
                ? h('div', { class: 'el-collapse-item__body' }, slots.default ? slots.default() : [])
                : null
          }
        )
      ])
  ...
  setup(props, { attrs }) {
    return () =>
      h(
        'div',
        {
          class: ['el-skeleton', { 'is-animated': props.animated }, attrs.class]
        },
        Array.from({ length: props.rows }).map((_, index) =>
          h('div', {
            key: `skeleton-row-${index}`,
            class: ['el-skeleton__item', index === 0 ? 'is-title' : '']
          })
        )
      )
  }
