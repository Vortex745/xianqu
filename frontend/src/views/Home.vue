...
import { animConfig, createAnimContext, cleanupContext, refreshScrollTriggers, shouldReduceMotion } from '@/utils/animations'
...
const handlePublishedRedirect = async () => {
  if (sessionStorage.getItem('xianqu_just_published') !== '1') return false
  sessionStorage.removeItem('xianqu_just_published')
  if (!onlyNearby.value) return false
  onlyNearby.value = false
  ElMessage.success('已为你展示全站商品，刚发布的宝贝更容易找到')
  return true
}
...
const initAnimations = () => {
  if (!pageContainer.value || shouldReduceMotion()) return

  animCtx = createAnimContext(pageContainer.value, () => {
...
.nav-search-bar {
  ...
  .mini-search-btn {
    ...
    &:hover {
      background: #f2d149;
      box-shadow: 0 10px 18px rgba(255, 223, 93, 0.22);
    }
...
.hero-section {
  ...
  text-align: center;
  background: linear-gradient(180deg, $primary 0%, rgba(255, 223, 93, 0.2) 60%, $bg-color 100%);
  ...
  .search-box-large {
    ...
    box-shadow: 0 8px 24px rgba(255, 218, 68, 0.25);
    transition:
    ...
    .btn-search-large {
      ...
      &:hover {
        background: #f0cf4b;
        box-shadow: 0 10px 20px rgba(255, 223, 93, 0.28);
      }
    ...
    &:focus-within {
      ...
      box-shadow: 0 16px 32px rgba(255, 218, 68, 0.32);
      border-color: rgba(255, 223, 93, 0.85);
...
.action-bar-wrapper {
  ...
  .filter-chip {
    ...
    &.active { background: #000; color: $primary; }
  }
...
.product-card {
  ...
  border-radius: 16px;
  overflow: hidden;
  border: 1px solid rgba(19, 24, 32, 0.04);
  transition:
    transform var(--motion-duration-normal) var(--motion-ease-out),
    box-shadow var(--motion-duration-normal) var(--motion-ease-out),
    border-color var(--motion-duration-fast) var(--motion-ease-out);
  ...
  &:hover {
    ...
    box-shadow: 0 16px 30px rgba(12, 18, 28, 0.08);
    border-color: rgba(255, 223, 93, 0.36);
