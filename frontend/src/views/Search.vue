  ...
  <div class="search-page" ref="searchPage">
    ...
    <div class="result-container" v-loading="loading">
          ...
          <div class="product-card">
            ...
            <div class="info-box">
              ...
              <div class="tags">
                ...
                <span class="tag" v-if="item.is_home_delivery">送货上门</span>
                <span class="tag" v-if="item.is_self_pickup">自提</span>
              </div>
      ...
      <div v-else-if="!loading" class="empty-state">
        ...
        <p>这里空空如也，换个词试试？</p>
        <span class="empty-tip">没有找到相关宝贝，您可以尝试减少关键词或更换分类</span>
      </div>
    </div>
  </div>
</template>
...
import { animConfig, createAnimContext, cleanupContext, refreshScrollTriggers, feedbackAnim } from '@/utils/animations'
...
const doSearch = async () => {
  ...
  try {
    ...
    // Refresh ScrollTrigger after content update
    nextTick(() => {
...
const initAnimations = () => {
  if (!searchPage.value) return

  animCtx = createAnimContext(searchPage.value, () => {
    // Header entrance
    gsap.from('.fixed-header', {
      y: -40,
      opacity: 0,
    ...
    // Filter panel slide down when opened
...
.filter-panel {
  ...
  .filter-row {
    ...
    .chip {
      ...
      &.active { background: $primary; color: #000; border-color: color.adjust($primary, $lightness: -5%); font-weight: bold; }
