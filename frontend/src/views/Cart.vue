<template>
  <div class="cart-page" ref="cartPage">
...
import { animConfig, createAnimContext, cleanupContext, shouldReduceMotion } from '@/utils/animations'
...
const clearCartItemInlineState = () => {
  if (!cartPage.value) return
  gsap.set('.cart-item-card', { clearProps: 'opacity,transform' })
}

const initAnimations = () => {
  if (!cartPage.value) return

  clearCartItemInlineState()
  if (shouldReduceMotion()) return

  animCtx = createAnimContext(cartPage.value, () => {
    ...
    gsap.from('.bg-orb', {
      scale: 0.8,
      opacity: 0,
      duration: animConfig.duration.dramatic,
      stagger: 0.2,
...
const refreshCartAnimations = () => {
  nextTick(() => {
    if (animCtx) cleanupContext(animCtx)
    initAnimations()
  })
}
