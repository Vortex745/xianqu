...
const fetchOrders = async () => {
  ...
  try {
    ...
    allOrders.value = (res.data || []).map(order => {
      ...
      // 买家头像修复
      if (order.user && order.user.avatar && !order.user.avatar.startsWith('http')) {
        order.user.avatar = 'http://127.0.0.1:8081' + order.user.avatar
      }
      return order
    })
    applyFilter()
...
.navbar {
  ...
  .left { display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 700; font-size: 14px; color: #657082; &:hover { color: #2f3948; } }
