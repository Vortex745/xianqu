...
// Init 初始化管理员账号 (后门接口，用于快速创建/重置管理员)
// 访问地址: http://localhost:8081/api/admin/init
func (a *AdminController) Init(c *gin.Context) {
	var existing models.User
	err := config.DB.Where("username = ?", "admin").First(&existing).Error
	...
	if err == nil {
		...
		c.JSON(http.StatusOK, gin.H{"message": "管理员密码已重置! 账号: admin, 密码: 1234"})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"message": "管理员创建成功! 账号: admin, 密码: 1234"})
}
