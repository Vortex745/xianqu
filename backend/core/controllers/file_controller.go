...
func prepareUploadPayload(srcFile multipart.File) ([]byte, string, error) {
	...
	if err != nil {
		...
		finalData, readErr := io.ReadAll(srcFile)
		if readErr != nil {
			return nil, "", readErr
		}

		return finalData, http.DetectContentType(finalData), nil
	}
	...
	if width > maxUploadImageDimension || height > maxUploadImageDimension {
		newImg = resize.Thumbnail(maxUploadImageDimension, maxUploadImageDimension, img, resize.Lanczos3)
	}
	...
	default:
		if err := jpeg.Encode(&buf, newImg, &jpeg.Options{Quality: compressedJPEGQuality}); err != nil {
			return nil, "", err
		}
	}
