...
func (ac *AnalyticsController) BehaviorTrend(c *gin.Context) {
	...
	err = config.DB.Raw(`
		...
		GROUP BY to_char(created_at AT TIME ZONE 'Asia/Shanghai', 'YYYY-MM-DD'), action
		ORDER BY day ASC
	`, startUTC, endUTC).Scan(&rows).Error
...
// HotProducts 热门商品 Top10（GORM 聚合，无方言差异）
...
func (ac *AnalyticsController) UserActivity(c *gin.Context) {
	...
	err = config.DB.Raw(`
		...
		GROUP BY
			EXTRACT(ISODOW FROM created_at AT TIME ZONE 'Asia/Shanghai'),
